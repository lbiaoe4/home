      ...
      <div class="card" style="max-width:920px; margin:0 auto;">
        ...
        <div class="card-b">
          ...
          <div class="card">
            ...
            <div class="card-b">
              <div class="muted small" id="uRoomReportHint" style="margin-bottom:10px;"></div>
              <div id="uRoomReportGames"></div>

              <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; margin-top:12px;">
                <span class="p-pill" id="uRoomReportScore">Placar: 0 x 0</span>
                <button class="btn2" id="uRoomReportClear">Limpar</button>
                <button class="btn2 primary" id="uRoomSave">Salvar reporte</button>
...
<div class="card" style="margin:14px 0 10px; background: rgba(255,255,255,.03); border:1px solid rgba(255,255,255,.08);">
  <div class="card-h" style="padding:12px 14px;">
    <div>
      ...
      <div class="muted small">Selecione templates (Mapa + Civ) por BO3/BO5/BO7.</div>
    </div>
  ...
  <div class="card-b" style="padding:12px 14px;">
    <div class="row">
      ...
      <div class="field" style="flex:1;">
        <label>Template BO3 (MD3)</label>
        <select class="select" id="evDraftTplBO3">
          <option value="">(nenhum)</option>
        </select>
      </div>

      <div class="field" style="flex:1;">
        <label>Template BO5 (MD5)</label>
        <select class="select" id="evDraftTplBO5">
          <option value="">(nenhum)</option>
        </select>
      </div>

      <div class="field" style="flex:1;">
        <label>Template BO7 (MD7)</label>
        <select class="select" id="evDraftTplBO7">
          <option value="">(nenhum)</option>
        </select>
        <div class="muted small" id="evDraftTplHint" style="margin-top:6px;">
          Se BO5/BO7 ficar vazio, o sistema usa BO3 como fallback.
        </div>
      </div>
    </div>
  </div>
...
regs.push({
  ...
  const roomReportHint = document.getElementById("uRoomReportHint");
  const roomReportGames = document.getElementById("uRoomReportGames");
  const roomReportScore = document.getElementById("uRoomReportScore");
  const roomReportClear = document.getElementById("uRoomReportClear");
  ...
  function renderTable(){
    ...
    tbody.querySelectorAll("[data-u-room]").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        ...
        // Draft link: SEMPRE baseado no match clicado
// Regra nova: no Draft, o jogador de MAIOR seed (menor número) deve ser o lado A.
// (Isso NÃO altera o lado A/B da partida para reporte/resultado; é só para o Draft.)
try { localStorage.setItem("lbi_last_draft_match_code", m.code || ""); } catch(e){}
const mySide = sideOf(m); // "A" | "B" (lado da partida)
const oppSide = (mySide === "A") ? "B" : "A";

// Definir role do Draft (a|b) baseado em seedA/seedB.
// Se seeds não existirem, cai no comportamento antigo: seu lado da partida.
let draftRole = (mySide || "A").toLowerCase(); // fallback
const sA = Number(m.seedA);
const sB = Number(m.seedB);
if (Number.isFinite(sA) && Number.isFinite(sB) && sA !== sB) {
  const draftAId = (sA < sB) ? m.participantAId : m.participantBId; // maior seed (menor número)
  draftRole = (draftAId === user.id) ? "a" : "b";
}
...
const params = new URLSearchParams({
  ...
  role
        ...
        roomDraftLink.href = `./draft.html?${params.toString()}`;

        // --- Reporte por game (Vitória/Derrota) ---
        const mdRaw = (m.md || defaultMdByRound(m.round) || "MD3").toUpperCase();
        const gamesTotal = Math.max(1, Number(String(mdRaw).replace(/[^0-9]/g,"")) || 3);
        const winsNeeded = Math.floor(gamesTotal/2) + 1;

        let reportedGames = Array.isArray(m.reportedGames) ? m.reportedGames.slice(0, gamesTotal) : [];
        while(reportedGames.length < gamesTotal) reportedGames.push(null);

        function calcScore(){
          let a=0,b=0;
          for(const v of reportedGames){
            if(v==="A") a++;
            if(v==="B") b++;
          }
          return {a,b};
        ...
        function updateScorePill(){
          if(!roomReportScore) return;
          const {a,b} = calcScore();
          roomReportScore.textContent = `Placar: ${a} x ${b}`;
        }
        function renderGames(){
          if(roomReportHint) roomReportHint.textContent = `Série ${mdRaw} (melhor de ${gamesTotal}) • Você é ${mySide} • Marque cada game como vitória/derrota.`;
          if(!roomReportGames) return;

          roomReportGames.innerHTML = Array.from({length: gamesTotal}).map((_,i)=>{
            const g = i+1;
            const v = reportedGames[i]; // "A" | "B" | null
            const mine = (v === mySide);
            const opp = (v === oppSide);
            return `
              <div class="card" style="margin:0 0 10px 0; padding:12px; display:flex; align-items:center; justify-content:space-between; gap:12px;">
                <div>
                  <div class="kicker">GAME ${g}</div>
                  <div class="muted small">${v ? (mine ? "Você marcou vitória" : "Você marcou derrota") : "Sem reporte"}</div>
                </div>
                <div style="display:flex; gap:10px;">
                  <button class="btn2 ${mine ? "primary":""}" data-rw="W" data-gi="${g}">Vitória</button>
                  <button class="btn2 ${opp ? "primary":""}" data-rw="L" data-gi="${g}">Derrota</button>
                </div>
              </div>
            `;
          }).join("");

          roomReportGames.querySelectorAll("[data-gi]").forEach(btn=>{
            btn.addEventListener("click", ()=>{
              const gi = Number(btn.getAttribute("data-gi")||"0");
              const rw = btn.getAttribute("data-rw"); // W/L
              if(!gi) return;
              reportedGames[gi-1] = (rw==="W") ? mySide : oppSide;
              renderGames();
              updateScorePill();
              roomMsg.textContent = "";
            });
        ...
        if(roomReportClear){
          roomReportClear.onclick = ()=>{
            for(let i=0;i<gamesTotal;i++) reportedGames[i]=null;
            renderGames();
            updateScorePill();
            roomMsg.textContent = "Reporte limpo.";
          };
        }
        ...
        roomSave.onclick = () => {
          const {a,b} = calcScore();
          const done = (a >= winsNeeded) || (b >= winsNeeded);
          if(!done){
            roomMsg.textContent = `Marque os games até alguém atingir ${winsNeeded} vitórias.`;
            return;
          }

          m.reportedGames = reportedGames.slice(0, gamesTotal);
          m.reportedScoreA = a;
          m.reportedScoreB = b;
...
if (key === "usuarios") {
  ...
  bindRowActions();
        ...
        const msg = document.getElementById("evMsg");
          ...
          function fillDraftTemplateSelects(selected = {}) {
            const sel3 = document.getElementById("evDraftTplBO3");
            const sel5 = document.getElementById("evDraftTplBO5");
            const sel7 = document.getElementById("evDraftTplBO7");
            if (!sel3 || !sel5 || !sel7) return;

            const templates = loadDraftTemplates().slice().sort((a,b)=>String(a?.name||"").localeCompare(String(b?.name||"")));

            function fill(sel, picked) {
              const prev = picked || "";
            ...
            fill(sel3, selected.bo3);
            fill(sel5, selected.bo5);
            fill(sel7, selected.bo7);
          }
        ...
        fillDraftTemplateSelects();
        document.getElementById("evDraftTplBO3")?.addEventListener("focus", () => fillDraftTemplateSelects());
        document.getElementById("evDraftTplBO5")?.addEventListener("focus", () => fillDraftTemplateSelects());
        document.getElementById("evDraftTplBO7")?.addEventListener("focus", () => fillDraftTemplateSelects());
...
const draftTplBO3 = (document.getElementById("evDraftTplBO3")?.value || "").trim();
const draftTplBO5 = (document.getElementById("evDraftTplBO5")?.value || "").trim();
const draftTplBO7 = (document.getElementById("evDraftTplBO7")?.value || "").trim();

if (draftEnabled && !draftTplBO3) {
  msg.textContent = "Draft habilitado: selecione pelo menos o Template BO3 (MD3).";
  return;
}
...
fillDraftTemplateSelects({
  bo3: e?.draftTemplates?.bo3 || "",
  bo5: e?.draftTemplates?.bo5 || "",
  bo7: e?.draftTemplates?.bo7 || ""
});

document.getElementById("evDraftTplBO3").value = e?.draftTemplates?.bo3 || "";
document.getElementById("evDraftTplBO5").value = e?.draftTemplates?.bo5 || "";
document.getElementById("evDraftTplBO7").value = e?.draftTemplates?.bo7 || "";
const dTpl = document.getElementById("evDraftTemplateId");
if(dEn) dEn.value = (e.draftEnabled ? "yes" : "no");
